...
public class GameLogic : MonoBehaviour
{
    ...
    private void SetGame()
    {
        ...
        // 게임이 시작되었을 때 Player1의 좌표는 2차원 평면상의 (4, 7)이다.

        P2.SetCoordinate(new Vector2Int(4, 1));
        // 게임이 시작되었을 때 Player2의 좌표는 2차원 평면상의 (4, 1)이다.

        turn = Enums.EPlayer.Player1;
        // 게임이 시작되었을 때 첫 번째 턴은 Player1 이 가져간다.  
    }

    public Vector2Int GetPawnCoordinate(Enums.EPlayer ePlayer)
    // 해당 Player 의 보드판에서의 좌표를 리턴한다. 
    {
    ...
    // 해당 Player 에게 남은 나무 판자의 개수를 리턴한다. 
    {
        Pawn targetPawn = GetTargetPawn(ePlayer);
        return targetPawn.GetPlankNum();
    }

    public List<Vector2Int> GetMoveablePawnCoords(Enums.EPlayer ePlayer)
    // 보드판의 경계와 Plank 의 유무를 판단하여 해당 Player가 상하좌우 중 갈 수 있는 좌표들을 리턴한다.  
    {        
        Pawn targetPawn = GetTargetPawn(ePlayer);
    ...
    // 보드판에서 Plank 가 놓일 수 있는 좌표들을 모두 리턴한다.  
    {
    ...
    // 인자로 받은 좌표를 기준으로 북쪽에 Plank 가 있는지를 판단하여 bool 값을 리턴한다.    
    {
        foreach (Plank plank in planks)
        {
    ...
    // 인자로 받은 좌표를 기준으로 남쪽에 Plank 가 있는지를 판단하여 bool 값을 리턴한다.  
    {
        foreach (Plank plank in planks)
        {
    ...
    // 인자로 받은 좌표를 기준으로 동쪽에 Plank 가 있는지를 판단하여 bool 값을 리턴한다.  
    {
        foreach (Plank plank in planks)
        {
    ...
    // 인자로 받은 좌표를 기준으로 서쪽에 Plank 가 있는지를 판단하여 bool 값을 리턴한다.   
    {
        foreach (Plank plank in planks)
        {
    ...
    // 해당 좌표에 Plank 를 둘 수 있는지를 판단하여 bool 값을 리턴한다.
    {
        foreach (Plank plank in planks)
        {
    ...
    // Player 의 사방이 Plank 로 막히는지를 판단한다.  
    {
        Pawn pawn;
